...
class PolygoneRegulier(Polygone):
    ...
    def __init__(self, centre, rayon, nombreSommets):
        ...
        for i in range(0, nombreSommets, 1): #computes the coordinates for all corners
            ...
            # Coordinate output
            (self.__sommets).append(X[i]) #adds an element from X list into self.__sommets
            (self.__sommets).append(Y[i]) #then adds an element from Y list into self.__sommets
            i += 1 #starts again for the next element of both lists
        Polygone.__init__(self, self.__sommets) #recalls the method from Polygon mother class
